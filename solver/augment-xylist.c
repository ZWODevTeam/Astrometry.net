...
int augment_xylist(augment_xylist_t* axy,
    ...
    if (axy->imagefn) {
        ...
        if (axy->use_source_extractor) {
			...
			sl_append(cmd,"zwo_starTool");
			sl_append(cmd, fitsimgfn);
			append_escape(cmd, xylsfn);
			
			// char t_str[64] = {0};
			// snprintf(t_str,"%sAA",xylsfn);
			// strcpy(xylsfn,t_str);
			xylsfn[strlen(xylsfn)-1] = '1';
			xylsfn[strlen(xylsfn)-2] = 't';
			
			char cmd_str[64] = {0};
			snprintf(cmd_str,64,"zwo_starTool %s %s",fitsimgfn,xylsfn);
